...
assert Data.test1 == con2.get('session-offloading-manager?command=test-function&type=session&value=' + session1)
assert Data.test2 == con1.get('session-offloading-manager?command=test-function&type=session&value=' + session2)

assert Data.cart2 == con3.get('shopping-cart?product=banana', headers={'X-session':session1})

assert Data.test3 == con2.get('session-offloading-manager?command=test-function&type=session&value=' + session1)
assert Data.test4 == con1.get('session-offloading-manager?command=test-function&type=session&value=' + session2)
...
assert Data.test5 == con3.get('session-offloading-manager?command=test-function&type=session&value=' + session1)
assert Data.test6 == con2.get('session-offloading-manager?command=test-function&type=session&value=' + session2)

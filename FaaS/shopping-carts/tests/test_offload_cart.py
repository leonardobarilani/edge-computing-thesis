...
assert Data.cart2 == con3.get('shopping-cart?product=banana', headers={'X-session':session1})[0]
assert Data.cart3 == con2.get('shopping-cart?product=apple', headers={'X-session':session1})[0]

assert Data.test2 == con2.get('session-offloading-manager?command=test-function&type=session&value=' + session1)[0]

assert Data.onload == con3.get('session-offloading-manager?command=force-onload')[0]

assert Data.test3 == con2.get('session-offloading-manager?command=test-function&type=session&value=' + session1)[0]

assert Data.cart4 == con2.get('shopping-cart?product=orange', headers={'X-session':session1})[0]
assert Data.cart5 == con3.get('shopping-cart?product=pear', headers={'X-session':session1})[0]
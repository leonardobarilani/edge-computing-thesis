...
assert Data.cart2 == con3.get('shopping-cart?product=banana', 
    headers={'X-session':session1,'X-session-request-id':str(uuid.uuid4())})[0]
assert Data.cart3 == con2.get('shopping-cart?product=apple', 
    headers={'X-session':session1,'X-session-request-id':str(uuid.uuid4())})[0]
...
assert Data.cart4 == con2.get('shopping-cart?product=orange', 
    headers={'X-session':session1,'X-session-request-id':str(uuid.uuid4())})[0]
assert Data.cart5 == con3.get('shopping-cart?product=pear', 
    headers={'X-session':session1,'X-session-request-id':str(uuid.uuid4())})[0]